...
(function(){
  ...
  class NotebookPlot{
    ...
    /* flecha desde (x0,y0) en la dirección (dx,dy) — para vectores directores/normales */
    vector(x0,y0,dx,dy,opts={}){
      const ctx=this.ctx;
      const color = opts.color || INK.purple;
      const p1=this.toPx(x0,y0), p2=this.toPx(x0+dx,y0+dy);
      ctx.save();
      ctx.strokeStyle=color;
      ctx.fillStyle=color;
      ctx.lineWidth=opts.lineWidth||2.8;
      ctx.lineCap='round';
      if(opts.dash) ctx.setLineDash(opts.dash);
      ctx.beginPath(); ctx.moveTo(p1.x,p1.y); ctx.lineTo(p2.x,p2.y); ctx.stroke();
      ...
      if(opts.label){
        ...
        ctx.fillStyle = color;
        ctx.textAlign = opts.labelAlign || 'left';
        ctx.fillText(opts.label, p2.x + (opts.labelDx ?? 8), p2.y + (opts.labelDy ?? -8));
        ctx.restore();
      }
      return p2;
    }

    /* recta completa que pasa por (x0,y0) con dirección (dx,dy), recortada al recuadro visible */
    lineThrough(x0,y0,dx,dy,opts={}){
      if(dx===0 && dy===0) return;
      let tMin=-1e9, tMax=1e9;
      if(dx!==0){
        const t1=(this.xMin-x0)/dx, t2=(this.xMax-x0)/dx;
        tMin=Math.max(tMin, Math.min(t1,t2)); tMax=Math.min(tMax, Math.max(t1,t2));
      }
      if(dy!==0){
        const t1=(this.yMin-y0)/dy, t2=(this.yMax-y0)/dy;
        tMin=Math.max(tMin, Math.min(t1,t2)); tMax=Math.min(tMax, Math.max(t1,t2));
      }
      const p1={x:x0+tMin*dx, y:y0+tMin*dy};
      const p2={x:x0+tMax*dx, y:y0+tMax*dy};
      this.segment(p1.x,p1.y,p2.x,p2.y,opts);
    }
    text(x,y,str,opts={}){
      const ctx=this.ctx;
      const p=this.toPx(x,y);
